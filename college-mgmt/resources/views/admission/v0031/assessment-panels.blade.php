        ...
        <div class="card border-0 shadow-sm">
            <div class="table-responsive">
                <table class="table table-sm align-middle mb-0">
                    <thead class="table-light"><tr><th scope="col">Panel</th><th scope="col">Type</th><th scope="col">Session</th><th scope="col">Evaluators</th><th scope="col">Capacity</th><th scope="col">Candidates</th><th scope="col">Status</th></tr></thead>
                    <tbody>
        ...
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('admission.assessment-panels.store') }}" class="vstack gap-2">
                    @csrf
                    <input aria-label="Panel name" class="form-control form-control-sm" name="name" placeholder="Panel name" required>
                    <select aria-label="Panel Type" class="form-select form-select-sm" name="panel_type">
                    ...
                    <select aria-label="Selection Session" class="form-select form-select-sm" name="selection_session_id"><option value="">Selection session</option>@foreach($sessions as $session)<option value="{{ $session->id }}">{{ $session->session_name }}</option>@endforeach</select>
                    <input aria-label="Capacity" class="form-control form-control-sm" type="number" name="capacity" value="20" min="1">
                    <input aria-label="Venue or room" class="form-control form-control-sm" name="venue" placeholder="Venue or room">
                    <input aria-label="Scheduled At" class="form-control form-control-sm" type="datetime-local" name="scheduled_at">
                    <select aria-label="Panel evaluators" class="form-select form-select-sm" name="evaluator_ids[]" multiple size="5">@foreach($evaluators as $evaluator)<option value="{{ $evaluator->id }}">{{ $evaluator->name }}</option>@endforeach</select>
