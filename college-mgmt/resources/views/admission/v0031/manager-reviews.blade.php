...
<div class="card border-0 shadow-sm mb-3">
    <div class="card-body">
        <form method="GET" class="row g-2 align-items-end">
            <div class="col-md-3"><label class="form-label small mb-1">Status</label><select aria-label="Status" name="status" class="form-select form-select-sm"><option value="">All Status</option>@foreach(['pending','resolved','dismissed'] as $status)<option value="{{ $status }}" @selected(request('status') === $status)>{{ ucfirst($status) }}</option>@endforeach</select></div>
            <div class="col-md-4"><label class="form-label small mb-1">Review Type</label><input aria-label="duplicate_review" name="review_type" value="{{ request('review_type') }}" class="form-control form-control-sm" placeholder="duplicate_review"></div>
            ...
            <div class="col-md-3 d-flex gap-1"><button class="btn btn-primary btn-sm flex-fill">Apply filters</button><a href="{{ route('admission.manager-reviews.index') }}" class="btn btn-outline-secondary btn-sm">Reset</a></div>
        </form>
    </div>
</div>
<div class="card border-0 shadow-sm">
    <div class="table-responsive">
        <table class="table table-sm align-middle mb-0">
            <thead class="table-light"><tr><th scope="col">Type</th><th scope="col">Finding</th><th scope="col">Action Required</th><th scope="col">Due</th><th scope="col">Status</th><th scope="col">Resolve</th></tr></thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <form method="POST" action="{{ route('admission.manager-reviews.resolve', $review) }}" class="d-flex gap-1">
                            @csrf @method('PATCH')
                            <input aria-label="Resolution" class="form-control form-control-sm" name="resolution_notes" placeholder="Resolution" required>
                            <button class="btn btn-sm btn-outline-success">Save resolution</button>
                        </form>
                        @endif
                    </td>
                </tr>
