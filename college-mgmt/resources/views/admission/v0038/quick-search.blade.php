...
<div class="card border-0 shadow-sm mb-3"><div class="card-body"><form method="GET" action="{{ route('admission.quick-search.index') }}" class="d-flex gap-2"><input name="q" value="{{ $query }}" class="form-control" placeholder="Search by name, phone, email, application number, offer number, or session"><button class="btn btn-primary">Search</button></form></div></div>
<div class="card border-0 shadow-sm"><div class="card-header bg-white fw-bold">Results</div><div class="table-responsive"><table class="table table-sm mb-0" aria-label="Admission quick search results"><thead><tr><th>Type</th><th>Match</th><th></th></tr></thead><tbody>@forelse($results as $result)<tr><td>{{ ucfirst($result['type']) }}</td><td>{{ $result['label'] }}</td><td><a class="btn btn-sm btn-outline-primary" href="{{ $result['url'] }}">Open</a></td></tr>@empty<tr><td colspan="3" class="text-muted text-center">Enter a search term to find admission records.</td></tr>@endforelse</tbody></table></div></div>
</div>
@endsection
