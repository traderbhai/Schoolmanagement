        ...
        <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
            <div>
                ...
                <div class="text-muted">
                    <strong>{{ $applicant->user->name ?? 'N/A' }}</strong>
                    &mdash; {{ $applicant->application_number }}
                    &nbsp;<span class="{{ $applicant->statusBadge }}">{{ $applicant->statusLabel }}</span>
                </div>
                <div class="text-muted small mt-1">
                    <i class="bi bi-mortarboard me-1"></i>{{ $applicant->program->name ?? 'N/A' }}
                    @if($applicant->batch)
...
@if($scores->isEmpty())
    <div class="alert alert-info"><i class="bi bi-info-circle me-2"></i>No scores recorded for this applicant yet.</div>
@else
...
<div class="row g-3 mb-4">
    ...
    <div class="col-md-3">
        <div class="card border-0 shadow-sm text-center">
            <div class="card-body py-3">
                <div class="fs-2 fw-bold text-warning">#{{ $meritListEntry->rank }}</div>
...
<div class="card border-0 shadow-sm mb-3">
    ...
    <div class="card-body">
        ...
        <div class="row g-2">
            ...
            <div class="col-md-3 col-sm-4">
                <div class="border rounded p-2 text-center">
                    ...
                    <div class="fw-bold">{{ $score->parameter_scores[$param->id] ?? '—' }} / {{ $param->max_score }}</div>
                </div>
            </div>
            @endforeach
        </div>
        ...
        <div class="mt-2 text-muted small">
            Scored by {{ $score->scoredBy->name ?? 'Unknown' }} on {{ $score->created_at->format('d M Y H:i') }}
        </div>
    </div>
</div>
@endforeach
