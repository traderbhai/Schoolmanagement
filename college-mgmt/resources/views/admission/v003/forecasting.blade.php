...
<div class="container-fluid py-4">
    ...
    @endif
    <div class="card"><div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th scope="col">Created</th><th scope="col">Program</th><th scope="col">Source</th><th scope="col">Rate</th><th scope="col">Gap</th></tr></thead><tbody>@forelse($snapshots as $row)<tr><td>{{ $row->created_at?->format('d M H:i') }}</td><td>{{ $row->program_id ?: 'All' }}</td><td>{{ $row->source ?: 'All' }}</td><td>{{ $row->expected_conversion_rate }}%</td><td>{{ $row->projected_gap }}</td></tr>@empty<tr><td colspan="5" class="text-muted text-center py-3">No forecast snapshots.</td></tr>@endforelse</tbody></table></div></div>
</div>
@endsection
