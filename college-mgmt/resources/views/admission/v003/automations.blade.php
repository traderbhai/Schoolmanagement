...
<div class="container-fluid py-4">
    ...
    <div class="row g-4">
        <div class="col-lg-4"><form method="POST" action="{{ route('admission.automations.store') }}" class="card" onsubmit="return confirm('Save this Admission automation rule? Confirm trigger, conditions, priority, affected records, message-safety dependencies, and execution-log review before activation.')">@csrf<div class="card-header"><div class="fw-semibold">Rule Builder</div><div class="small text-muted">Start with narrow conditions, then use the execution log to confirm the rule touched the expected records only.</div></div><div class="card-body vstack gap-3"><input aria-label="Rule name" class="form-control" name="name" placeholder="Rule name" required><input aria-label="lead_created" class="form-control" name="trigger" placeholder="lead_created" required><input aria-label="Rule priority" class="form-control" name="priority" type="number" value="100"><textarea aria-label="Rule conditions JSON" class="form-control" name="conditions_json" rows="3" placeholder='{"source":"web_form"}'></textarea><textarea aria-label="Rule actions JSON" class="form-control" name="actions_json" rows="5" required>[{"type":"score_lead"},{"type":"next_action","value":"Call within SLA"}]</textarea><button class="btn btn-primary" @disabled(! $canManageAutomations)>Save Automation</button></div></form></div>
        <div class="col-lg-8">
            <div class="card mb-4"><div class="card-header"><div class="fw-semibold">Rules</div><div class="small text-muted">Priority controls rule order; inactive rules are retained for audit but should not affect live work.</div></div><div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th scope="col">Name</th><th scope="col">Trigger</th><th scope="col">Priority</th><th scope="col">Active</th></tr></thead><tbody>@forelse($automations as $automation)<tr><td>{{ $automation->name }}</td><td>{{ $automation->trigger }}</td><td>{{ $automation->priority }}</td><td>{{ $automation->is_active ? 'Yes' : 'No' }}</td></tr>@empty<tr><td colspan="4" class="text-muted text-center py-3">No automations.</td></tr>@endforelse</tbody></table></div></div>
            <div class="card"><div class="card-header"><div class="fw-semibold">Recent Executions</div><div class="small text-muted">Check this after a run to confirm status, timing, and duplicate suppression.</div></div><div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th scope="col">Rule</th><th scope="col">Status</th><th scope="col">When</th></tr></thead><tbody>@forelse($executions as $execution)<tr><td>{{ $execution->automation?->name }}</td><td>{{ $execution->status }}</td><td>{{ $execution->created_at?->diffForHumans() }}</td></tr>@empty<tr><td colspan="3" class="text-muted text-center py-3">No executions.</td></tr>@endforelse</tbody></table></div></div>
        </div>
    </div>
</div>
@endsection
