        ...
        <form method="POST" action="{{ route('admin.fees.update', $fee) }}">
            ...
            <div class="row g-3 mb-3">
                ...
                <div class="col-md-6">
                    ...
                    <input aria-label="Fee Type" type="text" name="fee_type" class="form-control @error('fee_type') is-invalid @enderror" value="{{ old('fee_type', $fee->fee_type) }}" required>
                    @error('fee_type')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-3">
                    <label class="form-label">Amount (₹) <span class="text-danger">*</span></label>
                    <input aria-label="Amount" type="number" name="amount" class="form-control" value="{{ old('amount', $fee->amount) }}" step="0.01" required>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Semester No.</label>
                    <input aria-label="Semester Number" type="number" name="semester_number" class="form-control" value="{{ old('semester_number', $fee->semester_number) }}" min="1" max="12">
