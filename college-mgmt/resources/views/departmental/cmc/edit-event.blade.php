      ...
      <div class="row g-3">
        ...
        <div class="col-md-6">
          ...
          <select name="event_type" class="form-select" required>
            ...
            <option value="{{ $t }}" {{ old('event_type',$event->event_type)===$t?'selected':'' }}>{{ ucwords(str_replace('-',' ',$t)) }}</option>
            @endforeach
          </select>
        </div>
        <div class="col-md-6">
        ...
        <div class="col-md-4">
          ...
          <input type="number" name="seats" class="form-control" value="{{ old('seats',$event->seats) }}" min="1">
        </div>
        <div class="col-md-6">
          <label class="form-label small fw-semibold">Registration Deadline</label>
          <input type="date" name="registration_deadline" class="form-control" value="{{ old('registration_deadline',$event->registration_deadline?->format('Y-m-d')) }}">
        </div>
