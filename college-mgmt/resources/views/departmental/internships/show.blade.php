...
<div class="container-fluid py-4">
    ...
    @php $sColor = ['ongoing'=>'info','completed'=>'success','dropped'=>'secondary'][$internship->status] ?? 'secondary'; @endphp
            ...
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-body">
                    <dl class="row mb-0">
                        ...
                        <dd class="col-sm-9"><span class="badge bg-{{ $sColor }} fs-6">{{ ucfirst($internship->status) }}</span></dd>
                        ...
                        <dd class="col-sm-9">{{ $internship->stipend ? '₹'.number_format($internship->stipend,0) : '—' }}</dd>
                        ...
                        <dd class="col-sm-9">
                            @for($i=1;$i<=5;$i++)
                                <i class="bi bi-star{{ $i <= $internship->rating ? '-fill text-warning' : ' text-muted' }}"></i>
                            @endfor
                        </dd>
                        @endif
                    </dl>
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('cmc.internships.complete', $internship) }}">
                        ...
                        <div class="mb-2">
                            ...
                            <input type="date" name="end_date" class="form-control form-control-sm" required value="{{ now()->toDateString() }}">
                        </div>
                        <div class="mb-2">
                            <label class="form-label small fw-semibold">Supervisor Feedback</label>
                            <textarea name="feedback" rows="3" class="form-control form-control-sm" placeholder="Optional..."></textarea>
                        </div>
                        <div class="mb-3">
                            <label class="form-label small fw-semibold">Rating (1-5)</label>
                            <input type="number" name="rating" class="form-control form-control-sm" min="1" max="5" placeholder="e.g. 4">
                        </div>
