    ...
    <div class="row g-4">
        <div class="col-lg-6">
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body">
                    <p><strong>Student:</strong> {{ $feeDemand->student->user->name ?? $feeDemand->student->enrollment_number ?? 'N/A' }}</p>
                    <p><strong>Enrollment No:</strong> {{ $feeDemand->student->enrollment_number ?? 'N/A' }}</p>
                    <p><strong>Program:</strong> {{ $feeDemand->student->program->name ?? 'N/A' }}</p>
                    <p><strong>Term:</strong> {{ $feeDemand->term->name ?? 'N/A' }}</p>
                    <p><strong>Total Amount:</strong> ₹{{ number_format($feeDemand->total_amount, 2) }}</p>
                    <p><strong>Scholarship Deduction:</strong> ₹{{ number_format($feeDemand->scholarship_deduction ?? 0, 2) }}</p>
                    <p><strong>Final Amount:</strong> ₹{{ number_format($feeDemand->final_amount, 2) }}</p>
                    <p><strong>Penalty Amount:</strong>
                        @if(($feeDemand->penalty_amount ?? 0) > 0)
                            <span class="text-danger fw-semibold">₹{{ number_format($feeDemand->penalty_amount, 2) }}</span>
                        @else
        ...
        <div class="col-lg-6">
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body p-0">
                    @if($payments->isEmpty())
                        <div class="p-4 text-center text-muted">No payments recorded for this student.</div>
                    @else
                        <table class="table table-sm mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-end small">₹{{ number_format($p->amount_paid, 2) }}</td>
                                    <td class="small text-muted">{{ ucfirst($p->payment_method ?? '—') }}</td>
                                    <td>
