...
<div class="container-fluid py-3">
    ...
    <div class="row g-3">
        <div class="col-xl-7"><div class="card shadow-sm h-100"><div class="card-header py-2 d-flex justify-content-between"><span class="fw-semibold">PMC Priority Queue</span><a href="{{ route('academics.pmc.workbench') }}" class="btn btn-sm btn-outline-primary">Open</a></div><div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th>Work</th><th>Owner</th><th>Priority</th><th>Due</th></tr></thead><tbody>@foreach($priorityItems as $item)<tr><td><div class="fw-semibold">{{ $item->title }}</div><div class="small text-muted">{{ str_replace('_',' ', $item->work_type) }}</div></td><td>{{ $item->owner?->name ?? 'Unassigned' }}</td><td>{{ $item->priority }}</td><td>{{ optional($item->due_at)->format('d M') }}</td></tr>@endforeach</tbody></table></div></div></div>
