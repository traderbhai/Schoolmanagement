...
<div class="card shadow-sm mb-3">
    ...
    <div class="table-responsive"><table class="table table-sm align-middle mb-0">
        <thead><tr><th scope="col">Faculty</th><th scope="col">Assignment</th><th scope="col">Status</th><th scope="col">Faculty Response</th><th scope="col">PMC Review</th></tr></thead>
        <tbody>@forelse($acknowledgements as $ack)<tr>
            ...
            <td>
                <form method="POST" action="{{ route('academics.pmc.faculty-assignment-acknowledgements.respond', $ack) }}" class="d-flex flex-column gap-1">
                    @csrf @method('PATCH')
                    <select aria-label="Faculty response type" name="response_type" class="form-select form-select-sm">
                    ...
                    <input aria-label="Constraints raised" name="constraints_raised" class="form-control form-control-sm" placeholder="constraints, comma-separated">
                    <input aria-label="Faculty note" name="faculty_note" class="form-control form-control-sm" placeholder="Faculty note">
                    <button class="btn btn-sm btn-outline-primary">Submit faculty response</button>
                </form>
            </td>
            <td>
                <form method="POST" action="{{ route('academics.pmc.faculty-assignment-acknowledgements.review', $ack) }}" class="d-flex flex-column gap-1">
                    @csrf @method('PATCH')
                    <select aria-label="PMC review status" name="status" class="form-select form-select-sm">
                        <option value="accepted">accepted</option>
                        <option value="concern_accepted">concern accepted</option>
                        <option value="revision_required">revision required</option>
                        <option value="reassigned">reassigned</option>
                    </select>
                    <input aria-label="PMC review note" name="review_note" class="form-control form-control-sm" placeholder="Review note">
                    <button class="btn btn-sm btn-outline-success">Save PMC review</button>
                </form>
            </td>
...
<div class="card shadow-sm mb-3">
    ...
    <div class="table-responsive"><table class="table table-sm align-middle mb-0">
        <thead><tr><th scope="col">Faculty</th><th scope="col">Hours</th><th scope="col">Classes</th><th scope="col">Daily Max</th><th scope="col">Band</th><th scope="col">Status</th><th scope="col">Decision</th></tr></thead>
        <tbody>@forelse($loadReviews as $review)<tr>
            ...
            <td>{{ $review->status }}</td>
            <td>@if(!in_array($review->status, ['approved','approved_overload','rejected'], true))<form method="POST" action="{{ route('academics.pmc.faculty-load-reviews.decide', $review) }}" class="d-flex gap-1">@csrf @method('PATCH')<input type="hidden" name="status" value="{{ in_array($review->load_band, ['overload','critical'], true) ? 'approved_overload' : 'approved' }}"><input aria-label="Decision note" class="form-control form-control-sm" name="decision_note" placeholder="Decision note"><button class="btn btn-sm btn-outline-success">Approve load</button></form>@else {{ $review->decision_note ?: '-' }} @endif</td>
...
@endisset
<div class="row g-3">
    <div class="col-md-6"><div class="card shadow-sm"><div class="card-header py-2 fw-semibold">Faculty Preferences</div><div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th scope="col">Faculty</th><th scope="col">Type</th><th scope="col">Max/Day</th><th scope="col">Weekly</th></tr></thead><tbody>@foreach($preferences as $preference)<tr><td>{{ $preference->teacher?->user?->name }}</td><td>{{ $preference->faculty_type }}</td><td>{{ $preference->max_classes_per_day }}</td><td>{{ $preference->max_weekly_load }}</td></tr>@endforeach</tbody></table></div><div class="card-footer py-2">{{ $preferences->links() }}</div></div></div>
    <div class="col-md-6"><div class="card shadow-sm"><div class="card-header py-2 fw-semibold">Workload Rules</div><div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th scope="col">Rule</th><th scope="col">Normal</th><th scope="col">Overload</th><th scope="col">Active</th></tr></thead><tbody>@foreach($rules as $rule)<tr><td>{{ $rule->name }}</td><td>{{ $rule->normal_weekly_hours }}</td><td>{{ $rule->overload_threshold }}</td><td>{{ $rule->is_active ? 'yes' : 'no' }}</td></tr>@endforeach</tbody></table></div><div class="card-footer py-2">{{ $rules->links() }}</div></div></div>
</div>
