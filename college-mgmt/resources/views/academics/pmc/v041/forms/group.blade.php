<form method="POST" action="{{ route('academics.pmc.course-groups.store') }}" class="card shadow-sm">@csrf
    ...
    <div class="card-body vstack gap-2">
        ...
        <select class="form-select form-select-sm" name="program_id"><option value="">Select program</option>@foreach($selectorOptions['programs'] ?? [] as $program)<option value="{{ $program->id }}">{{ $program->code ?: $program->name }} - {{ $program->name }}</option>@endforeach</select>
        <select class="form-select form-select-sm" name="batch_id"><option value="">Select batch</option>@foreach($selectorOptions['batches'] ?? [] as $batch)<option value="{{ $batch->id }}">{{ $batch->code ?: $batch->name }} - {{ $batch->program?->code }}</option>@endforeach</select>
        <select class="form-select form-select-sm" name="term_id"><option value="">Select term</option>@foreach($selectorOptions['terms'] ?? [] as $term)<option value="{{ $term->id }}">{{ $term->name }} - {{ $term->program?->code }}</option>@endforeach</select>
        <select class="form-select form-select-sm" name="subject_id"><option value="">Select subject</option>@foreach($selectorOptions['subjects'] ?? [] as $subject)<option value="{{ $subject->id }}">{{ $subject->code ?: $subject->name }} - {{ $subject->name }}</option>@endforeach</select>
...
<form method="POST" action="{{ route('academics.pmc.course-groups.auto-build') }}" class="card shadow-sm mt-3">@csrf
    ...
    <div class="card-body vstack gap-2">
        <input class="form-control form-control-sm" name="title" placeholder="Build run title" value="PMC Auto Group Build">
        <input class="form-control form-control-sm" name="group_prefix" placeholder="Group prefix" value="Section">
        <select class="form-select form-select-sm" name="group_type" required>
            @foreach(['core_section','elective_group','lab_group','tutorial_group','project_group','combined_section'] as $type)<option value="{{ $type }}">{{ str($type)->headline() }}</option>@endforeach
        </select>
        <select class="form-select form-select-sm" name="strategy"><option value="balanced_capacity">Balanced capacity</option><option value="choice_priority">Choice priority</option><option value="lab_capacity">Lab capacity</option></select>
        <select class="form-select form-select-sm" name="program_id"><option value="">Select program</option>@foreach($selectorOptions['programs'] ?? [] as $program)<option value="{{ $program->id }}">{{ $program->code ?: $program->name }} - {{ $program->name }}</option>@endforeach</select>
        <select class="form-select form-select-sm" name="batch_id"><option value="">Select batch</option>@foreach($selectorOptions['batches'] ?? [] as $batch)<option value="{{ $batch->id }}">{{ $batch->code ?: $batch->name }} - {{ $batch->program?->code }}</option>@endforeach</select>
        <select class="form-select form-select-sm" name="term_id"><option value="">Select term</option>@foreach($selectorOptions['terms'] ?? [] as $term)<option value="{{ $term->id }}">{{ $term->name }} - {{ $term->program?->code }}</option>@endforeach</select>
...
<form method="POST" action="{{ route('academics.pmc.course-group-adjustments.store') }}" class="card shadow-sm mt-3">@csrf
    ...
    <div class="card-body vstack gap-2">
        ...
        <input class="form-control form-control-sm" name="strength_delta" placeholder="Strength delta" value="1">
        <textarea class="form-control form-control-sm" name="reason" placeholder="Adjustment reason" required></textarea>
