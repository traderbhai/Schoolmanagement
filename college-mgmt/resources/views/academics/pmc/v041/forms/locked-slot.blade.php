<form method="POST" action="{{ route('academics.pmc.locked-slots.store') }}" class="card shadow-sm">@csrf
    ...
    <div class="card-body vstack gap-2">
        ...
        <select class="form-select form-select-sm" name="teacher_id"><option value="">No specific faculty</option>@foreach($selectorOptions['teachers'] ?? [] as $teacher)<option value="{{ $teacher->id }}">{{ $teacher->user?->name ?? ('Teacher #' . $teacher->id) }}</option>@endforeach</select>
