<form method="POST" action="{{ route('academics.pmc.timetable-generator.generate') }}" class="card shadow-sm">@csrf
    ...
    <div class="card-body vstack gap-2">
        <input aria-label="Run title" class="form-control form-control-sm" name="title" placeholder="Run title" value="PMC Generated Timetable">
        <select aria-label="Strategy" class="form-select form-select-sm" name="strategy">@foreach(['balanced','student_compact','faculty_balanced','adjunct_priority','room_optimized'] as $strategy)<option value="{{ $strategy }}">{{ str($strategy)->headline() }}</option>@endforeach</select>
