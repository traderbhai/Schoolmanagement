    ...
    <div class="card shadow-sm mb-3">
        <div class="card-body py-2">
            <form class="row g-2 align-items-end">
                <div class="col-md-3"><label class="form-label small">Search</label><input class="form-control form-control-sm" name="search" value="{{ request('search') }}" placeholder="Snapshot type"></div>
                <div class="col-md-2"><label class="form-label small">Band</label><select class="form-select form-select-sm" name="band"><option value="">All</option>@foreach(['low','medium','high','critical'] as $band)<option value="{{ $band }}" @selected(request('band')===$band)>{{ ucfirst($band) }}</option>@endforeach</select></div>
                ...
                <div class="col-md-1"><label class="form-label small">Order</label><select class="form-select form-select-sm" name="direction"><option value="desc" @selected(request('direction','desc')==='desc')>Desc</option><option value="asc" @selected(request('direction')==='asc')>Asc</option></select></div>
