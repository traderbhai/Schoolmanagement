...
<div class="container-fluid py-3">
            ...
            <div class="col-xl-4">
                <form method="POST" action="{{ route('academics.pmc.planning.store') }}" class="card shadow-sm h-100">@csrf
                    ...
                    <div class="card-body vstack gap-2">
                        ...
                        <input class="form-control form-control-sm" name="academic_year" placeholder="Academic year, e.g. 2026-27">
                        <select class="form-select form-select-sm" name="program_id"><option value="">All programs</option>@foreach(($selectorOptions['programs'] ?? []) as $program)<option value="{{ $program->id }}">{{ $program->code }} - {{ $program->name }}</option>@endforeach</select>
                        <select class="form-select form-select-sm" name="term_id"><option value="">All terms</option>@foreach(($selectorOptions['terms'] ?? []) as $term)<option value="{{ $term->id }}">{{ $term->name }}</option>@endforeach</select>
                        <select class="form-select form-select-sm" name="owner_user_id"><option value="">Current user</option>@foreach(($selectorOptions['users'] ?? []) as $user)<option value="{{ $user->id }}">{{ $user->name }}</option>@endforeach</select>
                        <div class="row g-1"><div class="col"><input type="date" class="form-control form-control-sm" name="starts_at"></div><div class="col"><input type="date" class="form-control form-control-sm" name="ends_at"></div></div>
            ...
            <div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th>Readiness Item</th><th>Owner</th><th>Status</th><th>Completion</th><th>Blocker Action</th></tr></thead><tbody>
                @foreach($readinessItems as $item)
                    <tr>
                        ...
                        <td><form method="POST" action="{{ route('academics.pmc.semester-readiness.items.update', $item) }}" class="d-flex gap-1 align-items-center">@csrf @method('PATCH')<input type="number" class="form-control form-control-sm" name="completion_percent" value="{{ $item->completion_percent }}" min="0" max="100" style="max-width:76px"><select class="form-select form-select-sm" name="status"><option value="open" @selected($item->status==='open')>open</option><option value="in_progress" @selected($item->status==='in_progress')>in progress</option><option value="blocked" @selected($item->status==='blocked')>blocked</option><option value="done" @selected($item->status==='done')>done</option></select><input type="hidden" name="is_blocker" value="{{ $item->is_blocker ? 1 : 0 }}"><button class="btn btn-sm btn-outline-primary">Update</button></form></td>
    ...
    @if(!empty($studentPlans))
        ...
        <div class="card shadow-sm mb-3">
            ...
            <div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th>Student</th><th>Risk Signals</th><th>Mentor</th><th>Next Action</th><th>Quick Commands</th></tr></thead><tbody>
    ...
    @if(!empty($studentInterventions))
        <div class="card shadow-sm mb-3">
            ...
            <div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th>Intervention</th><th>Owner</th><th>Status</th><th>Action</th></tr></thead><tbody>
                @foreach($studentInterventions as $intervention)
                    <tr>
                        ...
                        <td><form method="POST" action="{{ route('academics.pmc.interventions.update', $intervention) }}" class="d-flex gap-1">@csrf @method('PATCH')<select class="form-select form-select-sm" name="status"><option value="mentor_contacted">Mentor contacted</option><option value="parent_contacted">Parent contacted</option><option value="remedial_assigned">Remedial assigned</option><option value="under_review">Under review</option><option value="resolved">Resolved</option><option value="escalated">Escalated</option></select><input class="form-control form-control-sm" name="evidence_note" placeholder="Evidence note"><button class="btn btn-sm btn-outline-primary">Update</button></form></td>
                    </tr>
                @endforeach
            </tbody></table></div>
    ...
    @if(!empty($parentEscalations))
        <div class="card shadow-sm mb-3">
            ...
            <div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th>Student</th><th>Guardian</th><th>Reason</th><th>Status</th></tr></thead><tbody>
    ...
    @if(!empty($deliveryCheckpoints))
        ...
        <div class="card shadow-sm mb-3">
            ...
            <div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th>Subject</th><th>Delivery</th><th>Risk Signals</th><th>Owner</th><th>Quick Action</th></tr></thead><tbody>
    ...
    @if(!empty($groupDeliveryTrackers))
        <div class="card shadow-sm mb-3">
            ...
            <div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th>Group</th><th>Faculty</th><th>Progress</th><th>Risk</th><th>Recommended Action</th></tr></thead><tbody>
    ...
    @if(!empty($sessionDeliveryLogs))
        <div class="card shadow-sm mb-3">
            ...
            <div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th>Session</th><th>Schedule</th><th>Status</th><th>Readiness</th></tr></thead><tbody>
    ...
    @if(!empty($remedialActions))
        <div class="card shadow-sm mb-3">
            ...
            <div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th>Action</th><th>Owner</th><th>Status</th><th>Update</th></tr></thead><tbody>
                @foreach($remedialActions as $action)
                    <tr>
                        ...
                        <td><form method="POST" action="{{ route('academics.pmc.remedial-planning.actions.update', $action) }}" class="d-flex gap-1">@csrf @method('PATCH')<select class="form-select form-select-sm" name="status"><option value="faculty_contacted">Faculty contacted</option><option value="makeup_scheduled">Makeup scheduled</option><option value="marks_collected">Marks collected</option><option value="under_review">Under review</option><option value="resolved">Resolved</option><option value="escalated">Escalated</option></select><input class="form-control form-control-sm" name="evidence_note" placeholder="Evidence note"><button class="btn btn-sm btn-outline-primary">Update</button></form></td>
                    </tr>
                @endforeach
            </tbody></table></div>
