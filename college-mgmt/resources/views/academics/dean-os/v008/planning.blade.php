        ...
        <div class="col-lg-4">
            <form method="POST" action="{{ route('academics.dean-os.planning.store') }}" class="card shadow-sm">@csrf
                ...
                <div class="card-body vstack gap-2">
                    <input aria-label="Plan title" class="form-control form-control-sm" name="title" placeholder="Plan title" required>
                    ...
                    <input aria-label="Academic year" class="form-control form-control-sm" name="academic_year" placeholder="Academic year">
                    <select aria-label="Plan status" class="form-select form-select-sm" name="status"><option value="draft">Draft</option><option value="branch_review">Branch Review</option><option value="dean_review">Dean Review</option><option value="approved">Approved</option><option value="published">Published</option><option value="revised">Revised</option></select>
                    <button class="btn btn-sm btn-primary" onclick="return confirm('Create this Dean academic plan? Confirm cycle type, academic year, current status, owners, and readiness evidence expectations before opening the planning record.')">Create plan</button>
                </div>
            </form>
        </div>
        <div class="col-lg-8">
            <div class="card shadow-sm"><div class="card-header py-2"><div class="fw-semibold">Planning Cycles</div><div class="small text-muted">Approval should follow readiness, not replace it.</div></div><div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th scope="col">Plan</th><th scope="col">Type</th><th scope="col">Status</th><th scope="col">Readiness</th><th scope="col">Action</th></tr></thead><tbody>
            @foreach($cycles as $cycle)<tr><td>{{ $cycle->title }}</td><td>{{ str_replace('_',' ', $cycle->cycle_type) }}</td><td><span class="badge text-bg-secondary">{{ $cycle->status }}</span></td><td>{{ $cycle->readiness_score }}%</td><td><form method="POST" action="{{ route('academics.dean-os.planning.approve', $cycle) }}">@csrf @method('PATCH')<input type="hidden" name="status" value="approved"><button class="btn btn-sm btn-outline-success" onclick="return confirm('Approve Dean planning cycle {{ addslashes($cycle->title) }}? Confirm readiness blockers, calendar/curriculum impact, workload evidence, and action ownership before approval.')">Approve planning cycle</button></form></td></tr>@endforeach
            </tbody></table></div><div class="card-footer py-2">{{ $cycles->links() }}</div></div>
        </div>
    </div>
    <div class="card shadow-sm mt-3"><div class="card-header py-2"><div class="fw-semibold">Readiness Blockers</div><div class="small text-muted">Every blocker needs an owner, due date, and action item before the plan is considered operationally ready.</div></div><div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th scope="col">Section</th><th scope="col">Blocker</th><th scope="col">Owner</th><th scope="col">Due</th><th scope="col" aria-label="Actions"></th></tr></thead><tbody>
        @foreach($blockers as $item)<tr><td>{{ str_replace('_',' ', $item->section) }}</td><td>{{ $item->title }}</td><td>{{ $item->owner?->name ?? 'Unassigned' }}</td><td>{{ optional($item->due_at)->format('d M Y') }}</td><td><form method="POST" action="{{ route('academics.dean-os.semester-readiness.action', $item) }}">@csrf<button class="btn btn-sm btn-outline-primary" onclick="return confirm('Create or reuse the action item for readiness blocker {{ addslashes($item->title) }}? Confirm owner, due date, source section, and resolution expectation before tracking it.')">Create action item</button></form></td></tr>@endforeach
