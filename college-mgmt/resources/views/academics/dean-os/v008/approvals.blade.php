...
<div class="container-fluid py-3">
    ...
    <div class="alert alert-info border-0 shadow-sm small mb-3">
        <div class="fw-semibold mb-1">Approval cockpit sequence</div>
        <div class="d-flex flex-wrap gap-2">
            <span class="badge text-bg-light border">1. Review pending and overdue</span>
            <span class="badge text-bg-light border">2. Check risk level and owner</span>
            <span class="badge text-bg-light border">3. Request evidence when unclear</span>
            <span class="badge text-bg-light border">4. Approve, return, reject, or escalate</span>
            <span class="badge text-bg-light border">5. Keep final decisions locked</span>
        </div>
    ...
    <div class="card shadow-sm"><div class="card-header py-2 small text-muted">Visible filter summary: all approval streams | sort: latest | export from Dean Reports</div><div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th scope="col">Approval</th><th scope="col">Type</th><th scope="col">Owner</th><th scope="col">Risk</th><th scope="col">Status</th><th scope="col">Decision</th></tr></thead><tbody>
        @foreach($items as $item)
            ...
            <tr>
                <td>{{ $item->title }}</td>
                <td>{{ str_replace('_',' ', $item->approval_type) }}</td>
