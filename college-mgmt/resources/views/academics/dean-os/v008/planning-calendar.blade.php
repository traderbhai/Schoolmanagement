...
<div class="container-fluid py-3">
    ...
    <div class="card shadow-sm"><div class="card-header py-2 small text-muted">Visible filter summary: event_type={{ request('event_type','all') }}, status={{ request('status','all') }}</div><div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th scope="col">Event</th><th scope="col">Type</th><th scope="col">Status</th><th scope="col">Starts</th><th scope="col">Source</th></tr></thead><tbody>@foreach($events as $event)<tr><td>{{ $event->title }}</td><td>{{ str_replace('_',' ', $event->event_type) }}</td><td>{{ $event->status }}</td><td>{{ optional($event->starts_at)->format('d M Y H:i') }}</td><td>{{ $event->source_type }} {{ $event->source_key }}</td></tr>@endforeach</tbody></table></div><div class="card-footer py-2">{{ $events->links() }}</div></div>
</div>
@endsection
