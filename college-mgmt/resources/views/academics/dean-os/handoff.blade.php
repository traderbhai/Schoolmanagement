...
<div class="container-fluid py-3">
    ...
    <div class="card shadow-sm"><div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th>Applicant</th><th>Status</th><th>Blockers</th><th>Notes</th><th>Updated</th></tr></thead><tbody>
        @forelse($records as $record)
            <tr><td><div class="fw-semibold">{{ $record->applicant_name ?? 'Applicant #' . $record->applicant_id }}</div><div class="small text-muted">{{ $record->application_number }}</div></td><td><span class="badge text-bg-light">{{ str_replace('_',' ', $record->status) }}</span></td><td class="small">@foreach((json_decode($record->blockers ?? '[]', true) ?: []) as $blocker)<span class="badge text-bg-warning me-1">{{ $blocker }}</span>@endforeach</td><td class="small text-muted">{{ $record->handoff_notes }}</td><td class="small">{{ $record->updated_at }}</td></tr>
        @empty
