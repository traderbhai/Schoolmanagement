...
| P2 | Admin Master Data / Academic Setup | In progress | Recent Stage 2 passes hardened semesters, batches, terms, academic calendar forms, role assignment. | Program/course/subject lifecycle locks, duplicate identifiers, academic-year windows, deletion/history preservation, inactive setup usage. | Admin master-data tests, academic structure tests, full suite. |
...
| P3 | Demo Seed Data And Browser Readiness | Release-ready | Goal 15 audited demo seeder wiring, APP_DEBUG/demo error behavior, sidebar scroll CSS, applicant mobile sidebar markup, seeded demo primary role pages, route smoke coverage, and localhost browser rendering. The local SQLite database was brought current with pending additive migrations after browser verification exposed a missing `notifications.deleted_at` column. Applicant mobile sidebar now uses the mobile sidebar class instead of inheriting fixed desktop sidebar behavior. Seeded demo primary portal pages open without Laravel/Whoops traces in focused tests, admin-accessible Admission/Academics/PMC/CoE/IQAC/Program Leadership/Course Delivery/Accounts/CMC pages render on localhost, and desktop/mobile sidebars are scrollable. | No known high-risk demo-mode error trace, broken primary seeded page, long-sidebar usability, or missing local migration gap remains in the final readiness surface. Re-audit only when new primary role dashboards, seeders, or layout shells are added. | Focused final readiness tests passed 37 / 115; expanded seeded/layout/route smoke passed 68 / 204; browser smoke passed for admin dashboard, Admission dashboard, Dean OS, PMC command, CoE, IQAC, Program Leadership, Course Delivery, Exam Cell, Accounts, CMC, Dean dashboard, and Program Chair dashboard; full suite passed 1345 / 8536. |
...
All 15 bounded completion goals are currently marked `Release-ready` with current full-suite evidence. The next bounded goal should be created only when a new feature area or regression is identified from current code evidence.
