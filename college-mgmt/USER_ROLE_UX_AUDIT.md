...
| --- | --- | --- | ---: | --- |
| Admin / Superuser | `admin@college.com` and `admin@demo.edu` | `admin.dashboard` | 106 | in_progress |
| Director | `director@college.com` | `director.dashboard` | 6 | in_progress |
| HOD / Department Head | `hod@college.com` | `hod.dashboard` | 7 | in_progress |

### Admission Department

| User Type | Demo User | Primary Landing | Status |
| --- | --- | --- | --- |
| Admission Head | `head@college.com` | `admission.dashboard` | in_progress |
| Admission Officer | `officer@college.com` | `admission.dashboard` | in_progress |
| Admission Manager | `admission.manager@college.com` | Admission workspace/manager scope | in_progress |
| Admission Counsellor | `counsellor@college.com` | Counsellor desk/workspace | in_progress |
| Admission Telecaller | `telecaller@college.com` | Calling desk / call queue | in_progress |
...
- Click and validate representative dashboard KPI cards and quick actions.
- Test filters, pagination, exports, and create/edit form entry on safe setup pages.
- Test mobile sidebar behavior for Admin/Director/HOD.
- Validate direct workflow usability for access-control/security screens.
...
Remaining Batch B work:
- Click and verify high-risk KPI/drilldown cards again from role-specific scopes.
- Test safe create/update flows: call disposition, reminder scheduling, document queue action, assessment panel view, offer/seat list filters, handoff list filters.
- Continue rendered-sidebar inspection for Admission Manager/Counsellor/Telecaller beyond the fixed Officer handoff mismatch.
...
- `/program-chair/students/at-risk` has filters and pagination, but no export/sort signal.
...
- Verification: focused PMC frontend test passed `5 tests / 120 assertions`; adjacent PMC regression passed `23 tests / 284 assertions`; full suite passed `1474 tests / 11635 assertions`.

Remaining Batch D work:
- Test safe timetable/course-allocation actions: filters, planner view switching, conflict drilldowns, saved views, exports, and approval/freeze form entry.
- Finish list-control/export checks for `/academics/pmc/course-allocation`, `/course-groups`, `/timetable-planner`, and legacy `/program-chair/students/at-risk`.
- Verify actual workflow completeness for allocation -> groups -> faculty assignment -> generator -> publish/freeze, not only page-load health.
- Inspect rendered scoped navigation for PMC Manager/Officer, Program Leader, Course Coordinator, and Faculty Mentor.
...
- Test safe workflows: readiness filters, marks/results queues, hall-ticket blockers, transcript lists, OBE readiness, attainment, feedback closure, audit compliance, exports.
- Inspect rendered scoped navigation for Exam Manager/Officer and IQAC Manager/Officer.
...
Severity: Medium  
Status: checked_no_blocker for Accounts/CMC sampled action entry
...
- Audit Admin-backed operations pages more deeply: Library, Hostel, Transport, Assets create/edit/list workflows.
- Test Accounts reconciliation/filter/export and CMC drive/company/event create/edit forms safely.
- Mobile browser checks for Accounts, CMC, and Admin-backed Operations dashboards/list pages.
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---|
| Admin / Director / HOD | done | sampled | sampled | sampled | missing | missing | missing | partial | Usable, heading coverage fixed through shared shell, but very broad and legacy-heavy. |
...
| Admission Officer/Manager/Counsellor/Telecaller | done | sampled | sampled | partial | missing | missing | missing | partial | Lower-role surfaces load; rendered Officer nav no longer exposes Handoff; deeper action flows still need testing. |
...
| Teacher Portal | done | done | sampled | partial | missing | missing | sampled | partial | Visible nav now returns 200; shared-shell heading coverage is fixed; missing-profile pages show operational empty states. |
| Student Portal | done | sampled | sampled | partial | missing | missing | sampled | partial | Academic Summary and shared-shell headings are fixed; fee-payment and feedback action availability now show explanations, with broader safe-action/ownership checks still open. |
| Parent Portal | done | sampled | sampled | partial | missing | missing | sampled | sampled | Basic boundaries sampled and shared-shell heading coverage fixed; deeper child-record ownership still needed. |
| Applicant Portal | done | sampled | sampled | partial | missing | missing | sampled | sampled | Staff route boundaries sampled and shared-shell heading coverage fixed; applicant checklist flow still needs submission testing. |
| Accounts | done | sampled | sampled | partial | missing | partial | sampled | partial | Routes/nav clean; shared-shell heading coverage fixed; export support exists on some pages only. |
| CMC | done | sampled | sampled | partial | missing | partial | sampled | partial | Routes/nav clean; shared-shell heading coverage fixed; primary CMC list exports are verified; create/edit workflows need action testing. |
| Library / Hostel / Transport / Assets | done | sampled | sampled | partial | partial | verified | sampled | partial | Admin-backed pages load with shared-shell heading coverage; Library, Hostel, Transport, and Assets primary list exports are verified; sampled sensitive lifecycle forms now have confirmation guards; Batch G mobile/table-overflow smoke is passing. |
