...
class MasterDataSeeder extends Seeder
{
    public function run(): void
    {
        ...
        try {
            ...
            // Build a unique set of (entry_id, teacher_id) pairs — attendance is per timetable_entry, not per subject
            $entryTeacherMap = [];
            foreach ($timetableEntries as $entry) {
                $entryTeacherMap[$entry->id] = $entry->teacher_id;
            }

            // Pre-load existing attendance keys for efficiency and to avoid unique constraint errors
            ...
            foreach ($students as $student) {
                foreach ($entryTeacherMap as $entryId => $teacherId) {
                    for ($daysAgo = 45; $daysAgo >= 1; $daysAgo--) {
                        ...
                        // Use deterministic seeding based on student+entry+date
                        ...
                        $existingKeys[$key] = true; // Mark as seen to prevent duplicate within this run
        ...
        try {
            ...
            foreach ($examSubjects as $subject) {
                ...
                foreach ($examDefs as $def) {
                    // semester_id is NOT NULL — get the first semester from the DB
